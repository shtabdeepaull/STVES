...
app.post("/api/vehicles", protect, requireRole("admin", "owner"), async (req, res) => {
  try {
    ...
    const vehicle = await Vehicle.create({
      ...
      safetyScore: 100,
      qrCode: `STVES-VEH:${normalizedRegistration}`,
    });
...
app.get("/api/vehicles/verify/:registrationNumber", protect, async (req, res) => {
  try {
    ...
    const verification = buildVehicleVerification(vehicle, driverAuthorization);
...
app.get("/api/licenses/verify/:licenseNumber", protect, async (req, res) => {
  try {
    ...
    const verification = buildLicenseVerification(license);
...
app.get("/api/violations/my", protect, requireRole("police", "admin"), async (req, res) => {
  try {
    const violations = await Violation.find({ officer: req.user._id })
      .populate("vehicle")
      .populate("driver", "name email role phone nid status")
      .populate("license")
      .populate("officer", "name email role badge station")
...
app.get("/api/qr/verify/:qrValue", protect, async (req, res) => {
  try {
    const qrValue = decodeURIComponent(req.params.qrValue || "").trim();
    ...
    if (qrValue.startsWith("STVES-VEH:")) {
      const plate = normalizePlate(qrValue.replace("STVES-VEH:", ""));
      ...
      const verification = buildVehicleVerification(vehicle);
      ...
      return res.json({
        ...
        type: "vehicle",
        vehicle,
        authorizedDrivers: assignments.map((item) => item.driver).filter(Boolean),
        verification,
      });
    }

    if (qrValue.startsWith("STVES-LIC:")) {
      const searchKey = normalizeLicenseKey(qrValue);
      ...
      await VerificationLog.create({
        ...
        searchType: "qr",
        searchValue: qrValue,
        driver: license.driver?._id || license.driver,
        license: license._id,
        result: verification.result,
        issues: verification.issues,
      });
